...
module.exports = function(grunt) {
  ...
  grunt.initConfig({
    ...
    meta: {
      ...
      cssFileDest: '<%= dist %>/<%= filename %>-csp-<%= pkg.version %>.css',
      banner: ['/*',
               ' * <%= pkg.name %>',
               ' * <%= pkg.homepage %>\n',
  ...
  grunt.registerTask('build', 'Create bootstrap build files', function() {
    ...
    var cssFiles = _.flatten(_.pluck(modules, 'cssFiles'));
    var styles = {
      css: [],
      js: []
    };
    cssFiles.forEach(processCSS.bind(null, styles, true));
    if (styles.js.length) {
      grunt.config('meta.cssInclude', grunt.config('meta.cssInclude') + styles.js.join('\n'));
